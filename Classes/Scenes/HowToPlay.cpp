...
Scene* HowToPlay::create()
{
    HowToPlay* scene = new HowToPlay();
    if (scene && scene->init())
    {
        scene->autorelease();
        return scene;
...
bool HowToPlay::init()
{
    ...
    auto background = Sprite::create("Background/a.png");
    background->setPosition(Vec2(visibleSize.width / 2, visibleSize.height *0.55));
   ...
   /* auto buttonRestart = ui::Button::create(buttonGreen2, buttonOrange);
    ...
    labelExit->setPosition(Vec2(visibleSize.width *0.5, visibleSize.height * 0.1));
